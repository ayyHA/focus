...
public class RedisServiceImpl implements IRedisService {
    ...
    public Boolean transChatZSetToRedis(String userId) {
        ...
        List<ChatEntity> sourceEntities = chatRepository.findByChatIdSourceId(userId);
        List<ChatEntity> targetEntities = chatRepository.findByChatIdTargetId(userId);
        ...
        if(sourceEntities!=null && sourceEntities.size()!=0){
            sourceEntities.forEach(e->{
                ...
                // 获取聊天用户的信息
                String chatUserId = e.getChatId().getTargetId();
                // 将分值最高的即日期最新的放入聊天集合中
                Double oldTimeStamp;
        ...
        if(targetEntities!=null && targetEntities.size()!=0){
            targetEntities.forEach(e->{
                ...
                Long timeStamp = e.getCreateAt().getTime();
                // 获取与userId的聊天用户的Id
                String chatUserId = e.getChatId().getSourceId();
                // 更新分值最高的即日期最新的于聊天集合中
                Double oldTimeStamp;
                if((oldTimeStamp = redisTemplate.opsForZSet().score(chatZSetKey,chatUserId))!=null){
                    if(timeStamp > oldTimeStamp)
    ...
    public Boolean transChatHistoryToRedis(String userId, String talkId) {
        ...
        List<ChatEntity> userIdEqSourceId = chatRepository.findByChatIdSourceIdAndChatIdTargetId(userId, talkId);
        // sourceId=talkId,targetId=userId;
        List<ChatEntity> userIdEqTargetId = chatRepository.findByChatIdSourceIdAndChatIdTargetId(talkId, userId);
        // 无聊天记录则返回false
    ...
    @Override
    public List<ChatDto> getChatHistoryFromRedis(String userId,String talkId) {
        // 拼接ChatHistoryKey
        String chatHistoryKey = RedisKeyUtil.LIST_KEY_CHAT_HISTORY + userId + ":" + talkId;
        // 不存在该key返回null
    ...
    // 更新历史聊天记录ChatHistory
    @Override
    public void updateChatHistoryInRedis(String userId,String talkId,ChatDto chatDto) {
        String chatHistoryKey = RedisKeyUtil.LIST_KEY_CHAT_HISTORY + userId + ":" + talkId;
